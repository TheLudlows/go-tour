...
Silvestri和Venturini [^24]提出了两个可变长度方案，我们选择了它们的最快版本称为VSEncoding）。VSEncoding使用动态编程对长度为1–14、16和32的块进行优化，从而优化了块长度。也就是说，给定数组中每个整数的整数对数，VSEncoding会找到一个分区，真正将总存储成本降至最低。我们希望VSEncoding提供比AFOR-2和AFOR-3更高的压缩率。
...
诸如BP32和SIMD-BP128之类的许多方案都要求在编码过程中计算整数对数。 如果天真地执行此步骤，则将占用大部分运行时间：整数对数的计算比诸如移位或加法之类的快速运算要慢。我们发现最好在x86平台上使用位扫描反向（bsr）汇编指令（因为只要x> 0，它就提供$log_2(x + 1)− 1)$。



对于二进制打包方案，我们必须在编码过程中确定整数$max_i(log_2(x_i + 1))$的最大对数。我们不在每个整数上计算一个整数对数，而是所有整数进行按位逻辑运算，然后计算结果的整数对数。由于以下等式，此快捷方式是可能的：$max_i(log_2(xi + 1))=log_2(∨i(xi + 1))$其中∨指的是按位逻辑或。
